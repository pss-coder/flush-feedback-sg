...
export default function QRURLClipboard({ shop, url}: {shop: ShopDB,url: string}) {
  ...
  const t = useTranslations('FeedbackQR');

  // Feedback URL
  const [maleFeedbackUrl, setMaleShopUrl] = useState(`http://` + url +  `/${localActive}/feedback/${shop.id}/male`);
  const [femaleFeedbackUrl, setFemaleShopUrl] = useState(`http://` + url + `/${localActive}/feedback/${shop.id}/female`);
  ...
  return (
    <div>
      <div className="absolute top-0 right-0 p-4">
        <LocaleSwitcher endpoint={`/feedback/${shop.id}`} />
      </div>
      ...
      <div className="w-full max-w-4xl p-6 bg-white rounded-lg shadow-lg">
        ...
        <div className="mb-8">
          <div className="flex flex-col items-center mb-4">
            <h2 className="text-xl font-semibold mb-2">{t("url")}</h2>
            <input
              type="text"
